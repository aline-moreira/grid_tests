...
for (( TAM = 356000000; TAM <= 1436000000; TAM = TAM + 90000000)); do 
	...
	echo "RUNNING PSRECORD ON $PROC PROCESS" && psrecord $PROC --interval 0.1 --log nao_pinado/stream_memory_$TAM.log --include-children;
	END=`date -u`;
	echo "FINISH the test in time $END";
	echo "$TAM;0;$INIT;$END" >> times_n_pinado.log;
done;
