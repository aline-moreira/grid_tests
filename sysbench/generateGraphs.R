...
energy_vm_dockerVM <- fromJSON(file="energy_vm_dockerVM.json")
energy_rede <- fromJSON(file="energy_rede.json")

energy_host_docker <- as.data.frame(energy_host_docker)
energy_vm_dockerVM <- as.data.frame(energy_vm_dockerVM)
energy_rede <- as.data.frame(energy_rede)
...
energy_vm_dockerVM$items.timestamps = as.POSIXct(energy_vm_dockerVM$items.timestamps, origin="1970-01-01",tz="UTC")

energy_rede$items.timestamps = as.POSIXct(energy_rede$items.timestamps, origin="1970-01-01",tz="UTC")
energy_rede$items.timestamps.1 = as.POSIXct(energy_rede$items.timestamps.1, origin="1970-01-01",tz="UTC")

energy_host_docker <- energy_host_docker %>% select(items.timestamps,items.values)
energy_host_docker$items.values.1 = NA

energy_vm_dockerVM <- energy_vm_dockerVM %>% select(items.timestamps,items.values)
energy_vm_dockerVM$items.values.1 = NA

energy_rede <- energy_rede %>% select(items.timestamps,items.values, items.values.1)

energy <- rbind(energy_host_docker, energy_vm_dockerVM)
energy <- rbind(energy, energy_rede)

names(energy) <- c("tempo","servidor1","servidor2")

# rm(energy_rede_host_docker)
rm(energy_vm_dockerVM)
...
mem <- as.data.frame(t(data.frame(
    ...
    c("MV",scaleFUN(mean(mem_vm$servidor1))),
    c("dockerMV",scaleFUN(mean(mem_dvm$servidor1)))
)))
...
mem <- as.data.frame(t(data.frame(
    ...
    c("MV",scaleFUN(mean(io_vm$servidor1))),
    c("dockerMV",scaleFUN(mean(io_dvm$servidor1)))
)))
colnames(mem)<- c("type","power")
rownames(mem)<- c("host","docker","MV","dockerMV")
