...
names(times) <- c("benchmark","plataforma","ram","start","end")
times$start <- as.POSIXct(times$start,tz="UTC", format="%d %b %Y %H:%M:%S %p")
times$end <- as.POSIXct(times$end, tz="UTC", format="%d %b %Y %H:%M:%S %p")
...
docker_idle <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==0] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==0]
)
...
docker_1 <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==1] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==1]
)
...
docker_50 <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==50] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==50]
)
...
docker_100 <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==100] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==100]
)
...
docker_150 <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==150] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==150]
)
...
docker_200 <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==200] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==200]
)
...
docker_250 <- energy %>% filter(
    energy$tempo >= times$inicio[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==250] &
        energy$tempo <= times$fim[times$benchmark=="STREAM" & times$plataforma=="docker" & times$ram==250]
)
...
p2 <- ggplot(data=somas, aes(x=cpus, y=Preço, color = ModeloCusto ))+
    geom_point()+
    geom_line()+
    theme_classic()+
    theme(
    ...
    scale_color_discrete(
        limits=c("aws","grid"),
        labels=c("Amazon Fargate", "Proposed Model")
    )
